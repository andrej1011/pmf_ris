...
<div class="sidebar">
    ...
    <ul class="nav flex-column">
        ...
        <li><a href="${pageContext.request.contextPath}/pacijent/buduciPregledi" class="nav-link active">&#128337; Budući pregledi</a></li>
        ...
        <div class="table-responsive">
            <c:choose>
                <c:when test="${empty pregledi}">
                    <p class="text-muted small p-3 mb-0">Nemate zakazanih pregleda.</p>
                </c:when>
                <c:otherwise>
                    <table class="table table-sm align-middle mb-0">
                        <thead class="table-light small text-uppercase">
                            <tr><th>Br.</th><th>Doktor</th><th>Datum i vreme</th><th>Status</th></tr>
                        </thead>
                        <tbody class="small">
                            <c:forEach items="${pregledi}" var="p">
                                <tr>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${p.status == 'ZAKAZAN'}"><span class="badge bg-info text-dark">ZAKAZAN</span></c:when>
                                            <c:when test="${p.status == 'U_TOKU'}"><span class="badge bg-warning text-dark">U TOKU</span></c:when>
